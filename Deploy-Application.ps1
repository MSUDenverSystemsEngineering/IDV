...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\idv_6_1_windows64_installer.exe" -Parameters "-q"
		...
		If (Test-Path "$env:Public\Desktop\IDV 6.1.lnk") {
			Remove-Item "$env:Public\Desktop\IDV 6.1.lnk" -Force
		}
		Else {
